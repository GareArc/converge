...
tenant, every workspace, every deployed cluster needs the same check, and the
...
code to change when a new tenant signs up.
...
`tenantIDs` stands in for a query against your own database — in a real
...
```
checking tenant acme
checking tenant globex
checking tenant acme
checking tenant globex
checking tenant acme
checking tenant globex
...
1. `acme` and `globex` printed together, in that order, before either name
...
3. `tenantIDs` ran again at the start of each round — nothing here remembers
   last round's list. A tenant you add to your own data between rounds shows
...
instead of taking nothing, it takes one value — the name of whichever tenant
converge wants checked right now. That name is an
[ID](../glossary.md#id): the name of one thing a job looks after. converge
doesn't tell your function what changed about `"acme"` or why
it's due; it just says `"acme"`, and your function looks at that tenant's
...
event, so if `"acme"` changes twice before you get to it, two messages are
waiting. Here there is no message to pile up, only a name on a list — if
`"acme"` needs checking twice before converge gets to it, converge checks it
once.
...
fails to load. converge keeps calling `tenantIDs` on its own, waiting a
little longer each time it fails, and if `tenantIDs` starts working again the
...
`tenantIDs` runs once at the start of every round, and converge waits for it
to return before checking the first tenant. Query ten thousand rows and take
